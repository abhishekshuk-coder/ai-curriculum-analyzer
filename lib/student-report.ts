...
export function buildStudentReportHtml({ result, sourceFilenames, analyzedAt }: StoredStudentAnalysis): string {
  const date = new Date(analyzedAt).toLocaleString();
  const sortedRoadmap = [...result.careerRoadmap].sort((a, b) => a.priority - b.priority);

  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<title>${escapeHtml(result.studentName)} — Career Readiness Report</title>
<style>
...
<body>
  <div class="sheet">
    <span class="eyebrow">AI Student Career Analysis Engine · Powered by Claude</span>
    <h1>${escapeHtml(result.studentName)}</h1>
    <p class="meta">
      Resume: ${escapeHtml(sourceFilenames.resume)}${sourceFilenames.transcript ? ` · Transcript: ${escapeHtml(sourceFilenames.transcript)}` : ""}
    ...
    <div class="health">
      <b>${result.healthScore}</b><span>/ 100 — ${escapeHtml(result.healthLabel)} career readiness</span>
    </div>
    ...
    <div class="tag-list">${result.detectedSkills.map((s) => `<span>${escapeHtml(s)}</span>`).join("")}</div>
    ...
    <h2>AI Resume Doctor™ Diagnosis</h2>
    ${result.profileDiagnostics.map((d) => `<p><strong>${escapeHtml(d.type)}:</strong> ${d.items.map(escapeHtml).join("; ")}</p>`).join("")}
    ...
    <p class="footer">Generated by the Universal AI Curriculum Relevance Analyzer — Student Module — analysis produced by Claude (Anthropic). For personal career planning use; verify recommendations against your own goals and circumstances.</p>
  </div>
</body>
</html>`;
}

export function downloadStudentReport(analysis: StoredStudentAnalysis) {
  const html = buildStudentReportHtml(analysis);
  const blob = new Blob([html], { type: "text/html;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  const safeName = analysis.result.studentName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
  a.href = url;
  a.download = `${safeName || "career"}-readiness-report.html`;
